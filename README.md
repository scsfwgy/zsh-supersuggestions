...
- **智能意图理解**：即使输入拼写错误或不完整的指令，LLM 也能理解你的真实意图并给出正确建议。例如输入 `toush` 会得到 `touch`，输入 `gut pohs` 会得到 `git push`；也支持自然语言描述，输入「adb如何获取设备列表」会得到 `adb devices`，输入「flutter健康自检」会得到 `flutter doctor`
...
| 项目 | 触发时机 | 知识来源 | 网络依赖 | 多候选 |
|------|----------|----------|----------|--------|
| [thefuck](https://github.com/nvbn/thefuck) | 命令失败后（事后） | 100+ 条内置规则匹配错误输出 | 无 | 单条修正 |
| [zsh-autosuggestions](https://github.com/zsh-users/zsh-autosuggestions) | 输入时实时（事中） | 命令历史 + 补全引擎 | 无 | 单条 inline |
| **zsh-supersuggestions（本项目）** | 按快捷键主动触发（事前/事中） | LLM API + 命令历史 | 需要 | 多候选菜单 / 多候选 cycling |

- **thefuck**：命令报错后输入 `fuck`，通过规则引擎匹配错误输出，返回修正命令。只能覆盖已知错误模式，无法处理未知场景。
- **zsh-autosuggestions**：输入时实时显示灰色 inline suggestion，按 `→` 接受。只展示历史中最近的一条匹配，不支持多候选循环。本项目依赖它并在此基础上扩展了 Ctrl+U / Ctrl+N 多候选 cycling。
- **zsh-supersuggestions**：通过 LLM 理解任意意图并生成全新命令（不限于历史中已有的），同时保留历史匹配能力。Ctrl+L 返回多候选垂直菜单，Ctrl+U / Ctrl+N 支持历史多候选循环，Ctrl+G 提供终端内 AI 问答。
